...
export class ExportedCommand extends Command {
    ...
    run(command: ParsedCommand){
        ...
        EmoteUtils.urlsFromEmojiString(command.message.content).then((emoteUrlArray) => {
            PatternUtils.urlsToPatternMessage(emoteUrlArray, command.message);
        }).catch((reason) => {
